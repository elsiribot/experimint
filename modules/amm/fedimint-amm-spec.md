...
A local failure to build/submit Tx2, or Tx2 being rejected by consensus, does **not** terminate the swap (fix pass 3, Important 1 and Important 3 — an earlier implementation treated Tx2 rejection as terminal, contradicting this section's own "permanently claimable" claim by stranding the balance): both re-enter `Tx1Accepted` after a bounded backoff, which re-reads the balance and retries the claim — self-healing exactly like a crash would. This is bounded by a fixed retry-attempt ceiling so a persistently broken client configuration eventually gives up (a terminal `Tx2Rejected`) rather than retrying forever, at the cost of that one pathological case needing a manual re-claim (the balance itself is never lost, only this operation's automatic pursuit of it).
...
- **Withdrawal is exact-or-reject, not slippage-tolerant** (fix pass 3, Important 6). `withdraw` sets `min_lo`/`min_hi` equal to the amounts it declares (a preview computed against the pool's reserves at call time), not a tolerance band beneath them: `WithdrawV0` settles at the pool's *current* reserves, so any concurrent operation on the pool — most commonly a swap — landing between the preview and settlement changes the actual payout on at least one leg, and a tolerance band under the declared amount can never be the binding constraint (settling below the declared amount on either leg makes `input < output` for that unit and core rejects with an opaque `UnbalancedTransaction` regardless of the tolerance; settling above it would silently forfeit the surplus, since the declared amounts — not the tolerance — are what is actually asked of core). Setting `min_lo`/`min_hi` to the exact declared amounts makes a reserve move between preview and settlement fail loudly with `SlippageExceeded` instead. Clients should simply retry a rejected withdrawal against a fresh preview.
