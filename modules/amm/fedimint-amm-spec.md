...
**Target:** `elsiribot/fedimint` branch `experimint-v0.11`, pinned at `51d011a47769c91aabe2ed6f1f62e91e53c50283` (workspace `0.12.0-alpha`, edition 2024)
...
| P15 | Instance registries are keyed by `ModuleInstanceId`: `ModuleRegistry<M> = BTreeMap<ModuleInstanceId, (ModuleKind, M)>`, and `ConfigGenParams::module_params` is the single source of truth for which instances run | `fedimint-core/src/module/registry.rs`; `fedimint-server/src/config/mod.rs` | **Many instances per kind are supported** (e.g. two `mintv2`, one per unit). The `assert!` against duplicates is on `ModuleInitRegistry` (kind → *init*): one implementation per kind, unrelated to instance count — §3.2 |
| P16 | `ConfigGenModuleArgs` is `{ network, disable_base_fees }` — the federation-wide half — alongside a typed per-instance `ServerModuleInit::Params` | `fedimint-server-core/src/init.rs` | `mintv2` uses it (`type Params = MintGenParams`) with `amount_unit` settable per instance, so each instance can denominate a different unit — §3.2 |
...
| `withdraw(position, shares)` | preview → build → submit → await → reissue both sides |

**State machines.** The swap is the only multi-step operation: `Tx1Submitted → Tx1Accepted → Tx2Submitted → Done`, with `Tx1Rejected` terminal. There is no refund state and no timeout, because a balance created by Tx1 is permanently claimable — a crash between the two steps is resumed, never unwound. Deposit and withdraw are single-transaction: `Submitted → Accepted | Rejected`.

A local failure to build/submit Tx2, or Tx2 being rejected by consensus, does **not** terminate the swap (fix pass 3, Important 1 and Important 3 — an earlier implementation treated Tx2 rejection as terminal, contradicting this section's own "permanently claimable" claim by stranding the balance): both re-enter `Tx1Accepted` after a backoff, which re-reads the balance and retries the claim — self-healing exactly like a crash would. **There is no retry-attempt ceiling and no terminal Tx2 failure state** (fix pass 4, Critical 2 — a bounded ceiling was briefly introduced between fix passes 3 and 4 and caught in re-review before shipping: bounding the retries reintroduced exactly the stranding bug this section's own first paragraph rules out, just moved one level down, since a terminal state is inactive and nothing would ever resume the claim). Retries instead run on an unbounded backoff (`background_backoff()`, never exhausting) forever: a real sleep on every retry rules out a hot loop, and the absence of a give-up state rules out abandonment.

**Slippage.** The client computes `min_out` from a fresh quote and the user's tolerance for `swap`, and `min_shares` from a fresh preview and tolerance for `deposit`; both must be recomputed immediately before submitting rather than reuse a cached figure. `withdraw` takes no slippage tolerance: it is exact-or-reject (fix pass 3, Important 6) — see §12.1.

**Recovery** re-derives candidate keys per §8 and scans the two recovery endpoints. Any `LpPosition` found is restored to the position list directly, as part of the scan. Any unclaimed `Balance` found is persisted locally, not claimed inline (fix pass 4, Critical 1 — an earlier version claimed inline from the framework's seed-restore hook, which panics: that hook runs before this module is registered in the client's module registry, and claiming requires submitting a transaction through it). The persisted balance is claimed once the module actually starts (`ClientModule::start`, which by construction only ever runs for a registered module), and again immediately by an on-demand rescan, which is already running on a registered module by the time it can be called.
...
- **Withdrawal is exact-or-reject, not slippage-tolerant** (fix pass 3, Important 6; completed in fix pass 4, Important 3). `withdraw` sets `min_lo`/`min_hi` **and** `max_lo`/`max_hi` equal to the amounts it declares (a preview computed against the pool's reserves at call time), not a tolerance band around them: `WithdrawV0` settles at the pool's *current* reserves, so any concurrent operation on the pool — most commonly a swap — landing between the preview and settlement changes the actual payout on at least one leg. A tolerance band under the declared amount can never be the binding constraint on the low side (settling below the declared amount on either leg makes `input < output` for that unit and core rejects with an opaque `UnbalancedTransaction` regardless of the tolerance), and `min_lo`/`min_hi` alone say nothing at all about the high side — settling above the preview used to pass silently, forfeiting the surplus, since the declared amounts (not any tolerance) are what is actually asked of core to mint back. `max_lo`/`max_hi` (added to `AmmInput::WithdrawV0` in fix pass 4, since the module's consensus version is `0.0` and unreleased) close that gap: a reserve move between preview and settlement in *either* direction now fails loudly with `SlippageExceeded`, and nothing is silently forfeited on either side. Clients should simply retry a rejected withdrawal against a fresh preview.
...
| Recovery-tweak squatting | `recipient_pk`/`owner_pk` name a destination, and core validates transaction signatures against **input** pubkeys only (`fedimint-server/src/consensus/transaction.rs`: inputs push `meta.pub_key`, signatures are validated, *then* outputs are processed). So an output's destination key was unauthenticated: an attacker could spend their own funds (cost: one `min_swap_in`) to create a `BalanceEntry`/`LpPosition` at a victim's key carrying a garbage `tweak`. The victim's own later credit accumulated into that record, and seed-only recovery (§8.2) could never re-derive the key from the garbage tweak — for an LP position, which is long-lived, that meant permanently unrecoverable | **Closed structurally** by a proof of possession: `SwapV0`/`DepositV0` carry a `pop`, a signature by the key they name over that output's own fields, verified in `process_output` before any database read (`fedimint-amm-common/src/pop.rs`). You cannot create a record at a key you do not control, so the first writer is always the owner. The signature covers the **tweak** (else it could be lifted from a pending transaction and re-paired with a garbage tweak) and the **pubkey** (else the x-only parity twin of a key would share a verification key while being a different database key). Replaying the whole tuple verbatim is harmless — it recreates the record the owner asked for, with the owner's own tweak |


> **Superseded mitigation.** This row previously described a *first-writer-wins* rule: the stored `tweak` was preserved whenever the record already existed, so only record creation could set it. That was argued sound because an attacker "cannot predict, and therefore cannot pre-create a record at, a victim's key" — with the stated caveat that it "would not hold" if the key could be **guessed or front-run**. The caveat was the operative case: `owner_pk` is plaintext in the submitted transaction, so a guardian observes it before ordering and can race a one-share deposit to the same key. Prediction was never required; observation sufficed. First-writer-wins has therefore been removed rather than patched — with a proof of possession it protects nothing an attacker can do, and preferring a stored tweak over an incoming one would only preserve an owner's own mistake against their own correction.
